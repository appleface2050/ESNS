...
function init_view_customer_service_company_integrated_file_convey() {
    /**
     * 文件传达
     * by 刘奕辰 2015-6-25
     */
...
window.show = function(news_id){
    ...
    httpRequest('/cp/get_company_news_by_id2', {id: news_id}).then(function (data) {
            ...
            if(data.result.type_flag=="news"){
                var content = $(data.result.content);

                var div = $('<div style="overflow: auto"></div>');
                div.append(content);
                var html = $('<div>').append(div).remove().html();
                bootbox.alert(html);
            }
            if(data.result.type_flag=="files"){

            }
            if(data.result.type_flag=="images"){
                //bootbox.dialog({
                //     // dialog的类名
                //     className: "my-modal"
                //});
                bootbox.alert('<div style="text-align: center;"><img style="max-width:100%;max-height:100%" src='+data.result.file_url_list[0]+'></div>');
            }
        });
    }
}