 ...
 */
function init_view_customer_service_create_company() {
    /**
     * 增加jpeg图片格式的上传
    ...
    function showView() {
        show_breadcrumb([
            {name: '创建公司'}
                 ...
                 httpRequest('/cp/get_all_big_company', {}).then(function (data) {
            if (!data.result) {
                return "";
            }
            show_breadcrumb([{name: data.result.name}]);
            return EJSTemplateRender('cp_manage/ejs/customer_service_create_company.ejs', {id:'', bigcompany_id:0,  'bigcompany_list': data.result});
        }).then(function (html) {
...
//                            upload_fileids = [last_get_upload_result.fileid];
    ...
    window.customer_service_create_company_callback = function (form) {
        /**
         回调函数
    ...
    window.customer_service_create_company_callback2 = function (form) {
        /**
         回调函数，上传企业logo
         by：王健 at：2015-06-22
         图片id使用 logoimage 的
    ...
    window.customer_service_create_company_callback3 = function (form) {
        /**
         回调函数，上传企业标题logo
         by：王健 at：2015-06-22
         图片id使用 logoimage 的
