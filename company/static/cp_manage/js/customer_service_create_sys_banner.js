...
function init_view_customer_service_create_sys_banner() {
    showView();
    function showView() {
        /**
         * 显示视图
         ...
         */
        show_breadcrumb([{name: '创建系统banner'}]);
        EJSTemplateRender('cp_manage/ejs/customer_service_create_sys_banner.ejs').done(function (html) {
...
//                            upload_fileids = [last_get_upload_result.fileid];
     ...
     * @type {string}
     */
    var url = '/nf/check_sys_banner_file_upload_status';
    var data = {
        fileid: fileid
    };
    httpRequest(url, data).then(function (data) {
     ...
     window.getFileInfo =function (fileid, cb, img_w, img_h) {
         ...
         * @type {string}
         */
        var url = '/nf/get_sys_banner_url_by_file';
        var data = {
            fileid: fileid
        };
        if (img_w && img_h) {
...
//        var url = '/nf/big_company/get_company_qn_upload_files_url';
        var url = '/nf/get_sys_banner_qn_upload_files_url';
        var data = {
            filetype: filetype,
            size: size,
            filename: filename
    ...
    window.customer_service_create_sys_banner_callback = function (form) {
        /**
         * * 优化创建系统banner
         by: 刘奕辰 at:2015-06-23
         */
        var obj = form.serializeArray();
        httpRequest('/cp/create_sys_banner', obj).then(function () {
